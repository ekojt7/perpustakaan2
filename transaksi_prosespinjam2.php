...
<html lang="en">
  ...
  <body>
	...
	$query=mysql_query("select * from buku where no_inv='$no_inv'");
	$d=mysql_fetch_array($query);
    $cekstok = mysql_query("select * from buku where no_inv='$no_inv' and jumlah=0");
    $hstok = mysql_fetch_array($cekstok);
	if(empty($d)){
	echo "<div id='form_alert' style='color:#DD1144' class='alert alert-success'>";
			echo "<a class='close' data-dismiss='alert' href='#'>x</a>Buku dengan $no_inv Tidak Ada Dalam Database!";
			echo "</div>";
	}
    else if (!empty($hstok)){
        echo "<div id='form_alert' style='color:#DD1144' class='alert alert-success'>";
			echo "<a class='close' data-dismiss='alert' href='#'>x</a>Buku dengan $no_inv Telah Habis Dipinjam!";
			echo "</div>";
    }
    else {
        ?>
        ...
        <div class="row">
            <form class="form-horizontal" action="transaksi_prosespinjam3.php" method="post">
  ...
  <div class="form-group">
    ...
    <div class="col-sm-6">
      <input type="text" class="form-control" id="no_inv" name="no_inv" value="<?php echo $d[no_inv]; ?>">
    </div>
  </div>
  <div class="form-group">
